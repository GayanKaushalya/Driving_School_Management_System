<?php
if (!class_exists('DBConnection')) { // Check if DBConnection class is already defined
	require_once('../config.php'); // Include configuration file
	require_once('DBConnection.php'); // Include DBConnection class file
}

// Class for managing system settings and user data
class SystemSettings extends DBConnection
{
	...
	// Load system information from database into session
	function load_system_info()
	{
		// if(!isset($_SESSION['system_info'])){
		$sql = "SELECT * FROM system_info"; // SQL query to select all system info
		$qry = $this->conn->query($sql); // Execute the query
		while ($row = $qry->fetch_assoc()) { // Fetch each row from the result
			$_SESSION['system_info'][$row['meta_field']] = $row['meta_value']; // Store system info in session
		}
		// }
	}

	// Update system information function
	...
	// Update system settings in the database and handle image uploads
	function update_settings_info()
	{
		...
		if (isset($_POST['content'])) // Check if content needs to be updated
			foreach ($_POST['content'] as $k => $v) { // Loop through content data
				file_put_contents("../{$k}.html", $v); // Write content to HTML files
			}
		...
		// Handle cover photo  upload function
	...
	// Set system information in session
